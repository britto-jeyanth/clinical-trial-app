            ...
            steps{
                script{
                    docker.withRegistry('https://build.hdap.gatech.edu'){
                        ...
                        def myImage = docker.build("scrubsonfhir:${env.BUILD_NUMBER}", "-f ./Dockerfile .")
        ...
        stage('Notify'){
            steps{
                script{
                    ...
                    rancher confirm: true, credentialId: 'rancher-server', endpoint: 'https://rancher.hdap.gatech.edu/v2-beta', environmentId: '1a7', environments: '', image: "build.hdap.gatech.edu/scrubsonfhir:${env.BUILD_NUMBER}", ports: '', service: 'ScrubsOnFHIR1/helloworld', timeout: 60
                }
            }
        }
    }
